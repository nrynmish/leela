...
const statusStyles: Record<Project["status"], string> = {
  ...
  done: "border-blue-500/20 bg-blue-500/10 text-blue-600 dark:text-blue-400",
};
  ...
  return (
    <Link href={`/projects/${project.id}`}>
        ...
        <CardHeader className="space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-2">
              ...
              <Badge
                variant="outline"
                className={cn("rounded-full capitalize", statusStyles[project.status])}
              >
                {project.status}
              </Badge>
            </div>
      ...
      <CardContent className="space-y-4">
        ...
        <Button variant="outline" className="w-full rounded-2xl" onClick={() => onOpen(project)}>
          View details
        </Button>
      </CardContent>
    </Card>
